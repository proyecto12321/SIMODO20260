...
const App = {
    ...
    async loadLocalOverrides() {
        // Trae todo el almacén clave-valor desde Supabase (base de datos
        // central, compartida por todos los dispositivos) y lo deja en
        // caché de memoria + localStorage como respaldo si no hay internet.
        await this.cargarKVDesdeSupabase();
    ...
    // ------------------------------------------------------------------
    // SUPABASE — almacén central clave/valor (tabla sgi_kv)
    // ------------------------------------------------------------------
    // Se usa una sola tabla genérica (key text, value jsonb) para no tener
    // que reescribir toda la app: cada "clave" de localStorage (evaluaciones,
    // plantillas, cuentas, fotos, etc.) se guarda como una fila en la nube,
    // así todos los dispositivos ven los mismos datos en tiempo real.
    _kvCache: {},

    async cargarKVDesdeSupabase() {
        try {
            const { data, error } = await supabaseClient.from('sgi_kv').select('key, value');
            if (error) throw error;
            this._kvCache = {};
            (data || []).forEach(fila => {
                this._kvCache[fila.key] = fila.value;
                try { localStorage.setItem(fila.key, JSON.stringify(fila.value)); } catch (e) {}
            });
        } catch (e) {
            console.error('No se pudo conectar a Supabase, se usará el respaldo local:', e);
        }
    },
    ...
    async intentarCargarInsigniaPersonalizada() {
        ...
        if (origen === 'carpeta') {
            ...
            this.storageRemove('sgi_logo');
            this.storageRemove('sgi_logo_origen');
    ...
    storageGet(key, fallback) {
        if (Object.prototype.hasOwnProperty.call(this._kvCache, key)) {
            return this._kvCache[key];
        }
        try {
    ...
    storageSet(key, value) {
        this._kvCache[key] = value;
        try { localStorage.setItem(key, JSON.stringify(value)); }
        catch (e) { console.error('No se pudo guardar localmente', key, e); }

        // Sincroniza con Supabase en segundo plano (no bloquea la UI).
        supabaseClient
            .from('sgi_kv')
            .upsert({ key: key, value: value, actualizado: new Date().toISOString() })
            .then(({ error }) => {
                if (error) console.error('No se pudo sincronizar con Supabase:', key, error);
            });
    },
    storageRemove(key) {
        delete this._kvCache[key];
        try { localStorage.removeItem(key); } catch (e) {}
        supabaseClient.from('sgi_kv').delete().eq('key', key).then(({ error }) => {
            if (error) console.error('No se pudo eliminar en Supabase:', key, error);
        });
    },

    // ------------------------------------------------------------------
    // SUPABASE STORAGE — archivos reales (fotos, logos, PDF, etc.)
    // ------------------------------------------------------------------
    // Nunca guardamos archivos grandes como texto base64 en la base de
    // datos (eso es lo que causaba caídas/errores). Cada archivo se sube
    // como binario al bucket "sgi-files" y solo guardamos su URL pública
    // (un texto corto) en la tabla sgi_kv.
    BUCKET_ARCHIVOS: 'sgi-files',

    async subirArchivoSupabase(carpeta, nombreBase, file) {
        try {
            if (!file) throw new Error('No se seleccionó ningún archivo');
            const extension = (file.name.split('.').pop() || 'bin').toLowerCase();
            const ruta = `${carpeta}/${nombreBase}_${Date.now()}.${extension}`;

            const { error: errorSubida } = await supabaseClient
                .storage
                .from(this.BUCKET_ARCHIVOS)
            ...
            console.error('Error subiendo archivo a Supabase Storage:', e);
            this.showToast('No se pudo subir el archivo. Se guardó una copia local mientras tanto.', 'danger');
            return null;
        }
    },

    // Convierte un archivo a base64 SOLO como respaldo local temporal
    // (para mostrar la vista previa al instante), nunca para guardarlo
    ...
    async guardarConfigInstitucion() {
        ...
        if (file) {
            ...
            const url = await this.subirArchivoSupabase('logo', 'institucion', file);
            if (url) {
                this.logoInstitucional = url;
    ...
    limpiarTodo() {
        ...
        this.storageRemove('sgi_evaluaciones_formulario1');
        this.storageRemove('sgi_evaluaciones_formulario2');
        this.storageRemove('sgi_plantillas');
        this.storageRemove('sgi_fotos');
        this.storageRemove('sgi_cuentas');
        this.storageRemove('sgi_logo');
        this.storageRemove('sgi_logo_origen');
        this.storageRemove('sgi_config_overrides');
    ...
    bindGlobalEvents() {
        ...
        document.getElementById('upload-photo-form').addEventListener('submit', async (e) => {
            ...
            const url = await this.subirArchivoSupabase('fotos', this.currentUser.dni, file);
            if (!url) return; // el error ya se mostró en subirArchivoSupabase
            this.fotos[this.currentUser.dni] = url;
            this.storageSet('sgi_fotos', this.fotos);
